...
#define immK() do { *imm = SEXT(BITS(i, 25, 20), 6); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  __VA_ARGS__ ; \
	...
	INSTPAT("0100000 ????? ????? 101 ????? 0110011" , sra 	 , R, R(rd) = (word_t)((sword_t)src1 >> src2)/* (sword_t)!!! */); 
	...
	INSTPAT("0000001 ????? ????? 000 ????? 0110011" , mul	 , R, R(rd) = (sword_t)src1 * (sword_t)src2); // auto truncate
	// INSTPAT("0000001 ????? ????? 001 ????? 0110011" , mulh	 , R, /* TODO */); 
	// INSTPAT("0000001 ????? ????? 010 ????? 0110011" , mulhsu	 , R, /* TODO */); 
	// INSTPAT("0000001 ????? ????? 011 ????? 0110011" , mulhu	 , R, /* TODO */); 
	INSTPAT("0000001 ????? ????? 110 ????? 0110011" , rem	 , R, R(rd) = (sword_t)src1 % (sword_t)src2);
	INSTPAT("0000001 ????? ????? 111 ????? 0110011" , remu	 , R, R(rd) = src1 % src2);

	INSTPAT("0000000 ????? ????? 000 ????? 0111011" , addw	 , R, R(rd) = SEXT(BITS(src1 + src2, 31, 0), 32));
	INSTPAT("0100000 ????? ????? 000 ????? 0111011" , subw	 , R, R(rd) = SEXT(BITS(src1 - src2, 31, 0), 32)); 
	INSTPAT("0000000 ????? ????? 001 ????? 0111011" , sllw	 , R, R(rd) = SEXT(BITS(src1 << src2, 31, 0), 32));
	INSTPAT("0000000 ????? ????? 101 ????? 0111011" , srlw	 , R, R(rd) = BITS(src1, 31, 0) >> src2);
	INSTPAT("0100000 ????? ????? 101 ????? 0111011" , sraw	 , R, R(rd) = SEXT(BITS((sword_t)src1 >> src2, 31, 0), 32));
	INSTPAT("0000001 ????? ????? 100 ????? 0111011" , divw	 , R, R(rd) = SEXT((int32_t)src1 / (int32_t)src2, 32)); 
	INSTPAT("0000001 ????? ????? 101 ????? 0111011" , divuw	 , R, R(rd) = SEXT((uint32_t)src1 / (uint32_t)src2, 32));
	INSTPAT("0000001 ????? ????? 000 ????? 0111011" , mulw	 , R, R(rd) = SEXT((int32_t)src1 * (int32_t)src2, 32)); 
	INSTPAT("0000001 ????? ????? 110 ????? 0111011" , remw	 , R, R(rd) = SEXT((int32_t)src1 % (int32_t)src2, 32)); 
	INSTPAT("0000001 ????? ????? 111 ????? 0111011" , remuw	 , R, R(rd) = SEXT((uint32_t)src1 % (uint32_t)src2, 32)); 
	...
	/* K-type */
	INSTPAT("000000 ?????? ????? 001 ????? 0010011" , slli   , K, R(rd) = src1 << imm);
	INSTPAT("000000 ?????? ????? 001 ????? 0011011" , slliw  , K, R(rd) = SEXT(BITS(src1 << imm, 31, 0), 32)); // shamt[5] === 0
	INSTPAT("000000 ?????? ????? 101 ????? 0010011" , srli   , K, R(rd) = src1 >> imm);
	INSTPAT("000000 ?????? ????? 101 ????? 0011011" , srliw  , K, R(rd) = BITS(src1, 31, 0) >> imm); // shamt[5] === 0
	INSTPAT("010000 ?????? ????? 101 ????? 0010011" , srai   , K, R(rd) = (sword_t)src1 >> imm);
	INSTPAT("010000 ?????? ????? 101 ????? 0011011" , sraiw  , K, R(rd) = SEXT(BITS((sword_t)src1 >> imm, 31, 0), 32));
